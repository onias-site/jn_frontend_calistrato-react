...
const PanelCodeHighlight = ({ children, title, codeHighlight, id, className = '', cssClassTitle =''}: PanelCodeHighlightProps) => {
    const [toggleCode, setToggleCode] = useState(false);
    return (
        <div className={`panel ${className}`} id={id}>
            <div className="mb-5 flex items-center justify-between">
                <h5 className={cssClassTitle || "text-lg font-semibold dark:text-white-light"}>{title}</h5>
            </div>
            {children}
            {toggleCode && (
                <CodeHighlight>
